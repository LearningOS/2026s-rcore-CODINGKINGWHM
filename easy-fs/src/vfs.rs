...
impl Inode {
    ...
    pub fn unlinkat(&self, name: &str) -> isize {
        ...
        let target_inode_id = self.read_disk_inode(|root_inode| self.find_inode_id(name, root_inode));
        ...
        let nlink = target.modify_disk_inode(|disk_inode| {
            disk_inode.nlink -= 1;
            disk_inode.nlink
        });
